...
async def create_entity(payload: BusinessEntityCreate, request: Request) -> Dict[str, Any]:
    ...
    if await db.business_entities.find_one({"short_name": payload.short_name}, {"_id": 0}):
        raise HTTPException(status_code=409, detail="Short name entitas sudah digunakan")
    entity = payload.model_dump()
    entity.update({
        "id": new_id("ent"), "status": "active",
        "created_by": actor["name"], "created_at": now_iso(), "updated_at": now_iso(),
    })
    await db.business_entities.insert_one(entity)
    await audit(actor["name"], "entity_created", "business_entity", entity["id"], entity)
    return safe_doc(entity)
