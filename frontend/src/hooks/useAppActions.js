...
export function useAppActions(state) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (!selectedCustomer && dash.data.customers?.[0]) {
  ...
  }, [selectedCustomer, user?.role, selectedEntity]);
  ...
  const addToCart = (product) => {
    setCart((current) => {
      ...
      return [...current, { product, quantity: Math.min(10, Math.max(1, product.available_qty || 1)), unit: product.base_unit }];
    });
    setNotice(`${product.name} masuk draft sales order.`);
  };
