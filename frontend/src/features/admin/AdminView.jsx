...
export default function AdminView({
  ...
  const [product, setProduct] = useState({ sku: "", name: "", category: "Batik", variant: "Regular", color: "", motif: "", grade: "A", supplier: "", base_unit: "meter", price: 0, harga_pokok: 0, gramasi: 0, lebar: 0, uom_conversions: [] });
  ...
  const emptyProduct = { sku: "", name: "", category: "Batik", variant: "Regular", color: "", motif: "", grade: "A", supplier: "", base_unit: "meter", price: 0, harga_pokok: 0, gramasi: 0, lebar: 0, uom_conversions: [] };
  ...
  const loadProductForEdit = (row) => {
    setProduct({
      ...
      gramasi: Number(row.gramasi || 0), lebar: Number(row.lebar || 0), uom_conversions: row.uom_conversions || [],
    });
    setEditingProductId(row.id);
  };

  const tabs = [
    ["entities", "Entities"], ["products", "Product"], ["customers", "Customer"], ["warehouses", "Warehouse"], ["uoms", "UOM"], ["settings", "Pengaturan"], ["templates", "Templates"], ["permissions", "Permissions"], ["audit", "Audit"], ["users", "Users"],
  ];
      ...
      <section className="grid gap-3 lg:grid-cols-[360px_1fr]">
        ...
        <div className="section-card">
          ...
          {tab === "products" && <div className="grid gap-2">
            {[["sku", "SKU"], ["name", "Nama produk"], ["category", "Kategori"], ["variant", "Varian"], ["color", "Warna"], ["motif", "Motif"], ["grade", "Grade"], ["supplier", "Supplier"], ["base_unit", "Base unit"], ["price", "Harga jual"], ["harga_pokok", "Harga pokok (HPP)"], ["gramasi", "Gramasi (gsm)"], ["lebar", "Lebar (meter)"]].map(([key, ph]) => { const isNum = ["price", "harga_pokok", "gramasi", "lebar"].includes(key); return <input key={key} data-testid={`admin-product-${key}-input`} className="field" type={isNum ? "number" : "text"} placeholder={ph} value={product[key] ?? ""} onChange={(e) => setProduct({ ...product, [key]: isNum ? Number(e.target.value) : e.target.value })} />; })}
