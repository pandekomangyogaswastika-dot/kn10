...
export const PAGE_META = {
  ...
  payables:               { kicker: "Pembelian",      title: "Hutang Supplier (AP)" },
  ...
  "cs-coa":               { kicker: "Keuangan",       title: "Chart of Accounts" },
  "cs-gl":                { kicker: "Keuangan",       title: "Jurnal / General Ledger" },
  "cs-bank":              { kicker: "Keuangan",       title: "Bank Accounts" },
  "cs-pajak":             { kicker: "Keuangan",       title: "Pajak (PPN / PPH)" },
  "cs-ar":                { kicker: "Keuangan",       title: "AR / Piutang & Denda" },
...
const NAV_STRUCTURE = [
  ...
  // ── 2. PEMBELIAN ─────────────────────────────────────────────────────────────
  {
    ...
    items: [
      ...
      { id: "purchase-requisitions",  label: "Purchase Requisition",    icon: ClipboardCheck, roles: ["admin", "manager", "sales", "warehouse"] },
      ...
      { id: "vendor-bills",           label: "Tagihan Supplier",        icon: Receipt,       roles: ["admin", "manager", "sales", "warehouse"] },
      { id: "landed-cost",            label: "Landed Cost (HPP)",       icon: Ship,          roles: ["admin", "manager", "sales", "warehouse"] },
      { id: "input-tax",              label: "Faktur Pajak Masukan",    icon: Percent,       roles: ["admin", "manager", "sales", "warehouse"] },
      { id: "payables",               label: "Hutang Supplier (AP)",    icon: Landmark,      roles: ["admin", "manager"] },
  ...
  // ── 5. KEUANGAN ─────────────────────────────────────────────────────────────
  {
    ...
    roles:      ["admin", "manager"],
    items: [
      { id: "cs-coa",     label: "Chart of Accounts",    icon: BookOpen,     roles: ["admin", "manager"], comingSoon: true },
      { id: "cs-gl",      label: "Jurnal / GL",           icon: FileStack,    roles: ["admin", "manager"], comingSoon: true },
      { id: "cs-bank",    label: "Bank Accounts",        icon: CreditCard,   roles: ["admin", "manager"], comingSoon: true },
      { id: "cs-pajak",   label: "Pajak (PPN / PPH)",    icon: Percent,      roles: ["admin", "manager"], comingSoon: true },
      { id: "cs-ar",      label: "AR / Piutang & Denda", icon: TrendingDown, roles: ["admin", "manager"], comingSoon: true },
...
// ─── BUILD GROUPED NAVIGATION (dipanggil di App.js) ──────────────────────────
/**
 * Kembalikan struktur navigasi yang sudah di-filter sesuai role.
 * Output: array of { type: "standalone"|"group", ... }
 *   - standalone items langsung masuk jika role match
 *   - groups masuk jika minimal 1 item dalam group cocok dengan role
 *     (items di dalam group juga di-filter)
 */
export function buildNavGroups(role) {
  ...
  for (const entry of NAV_STRUCTURE) {
    ...
    if (entry.type === "standalone") {
      ...
      const filtered = entry.items.filter(item => item.roles.includes(role));
...
/**
 * Default landing view per role setelah login.
 */
...
/**
 * Default nav item ID per role (untuk highlight sidebar saat pertama login).
 */
export function defaultNavIdForRole(role) {
  if (role === "admin")     return "admin";
  if (role === "warehouse") return "wms-stok";
  if (role === "manager")   return "reports";
  return "sales";
}
