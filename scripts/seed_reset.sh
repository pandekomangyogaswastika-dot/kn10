...
echo -e "\n${CYAN}[2/4] [GATE] Contract verifier (nama koleksi)...${RESET}"
...
echo -e "\n${CYAN}[4/4] [GATE] Data-integrity verifier (clean seed)...${RESET}"
...
if [ $CONTRACT_RC -eq 0 ] && [ $INTEG_RC -eq 0 ] && [ $APIC_RC -eq 0 ]; then
  ...
  echo -e "  ${RED}${BOLD}✗ GATE GAGAL — contract=$CONTRACT_RC api_contract=$APIC_RC integrity=$INTEG_RC${RESET}"
